...
} = require("../models/repositories/wallet.repo");
...
class TransactionService {
  static getAllTransactions = async ({ walletId, options, search }) => {
    ...
    const type =
      options.type !== "all" ? options.type : { $in: ["income", "expense"] };
  ...
  static createTransaction = async ({ userId, walletId, transaction }) => {
    ...
    const foundCategory = await categoryModel
      .findOne({ _id: transaction.category })
      .lean();
  ...
  static updateTransaction = async ({
    ...
    if (update.category) {
      const foundCategory = await categoryModel
        .findOne({ _id: update.category._id })
        .lean();
      if (!foundCategory) {
        throw new BadRequestError({
          data: {
    ...
    try {
      ...
      const { financial_plans: plans } = await walletModel
        .findOne({ _id: walletId })
        .populate({
      ...
      if (
        ...
        plans.forEach(async (plan) => {
          const planCategories = plan.attributes.categories.map((category) =>
            category.toString()
          );
